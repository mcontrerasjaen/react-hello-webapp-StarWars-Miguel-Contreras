...
export const DetallesSpecie = ({ info, volver }) => {
     const { store, dispatch } = useGlobalReducer();

    if (!info) return null;
    
    // Extraemos los datos para asegurar que lleguen bien
    const specie = info.data || info;
    const esFavorito = store.favoritos.some(f => f._id === specie._id);
        ...
        <div className="container mt-5 p-5 bg-dark text-light rounded border border-warning shadow-lg min-vh-75">
            ...
            <div className="row align-items-center">
                ...
                <div className="col-md-7 ps-md-5">
                    <h1 className="text-warning display-3 fw-bold mb-4">{specie.name}</h1>
                    <hr className="bg-warning mb-4" style={{ height: "3px", opacity: "1" }} />
                    
                    <p className="fs-4 lh-base" style={{ textAlign: "justify", color: "#f8f9fa" }}>
                        {specie.description || "No hay descripción disponible para esta specie."}
                    </p>
